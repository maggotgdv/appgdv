 		...
 		imgPicker: function(options) {
	    	...
	    	, saveImg = function(image) {
	    		...
	    		var mask = $('.cropper-mask')
					...
					data = {
						action: 'save',
						id:document.getElementById("random").value,
				...
				success:function(response) {
					...
					if (obj.success) {
						console.log("sucess");
						if (options.attr == 'src'){
							$(options.el).attr('src', obj.data + timestamp() );
							$(options.el).attr('value', obj.data);
							 console.log("exito");
						}
						else if ( $(options.el).length ){
							$(options.el).attr(options.attr, obj.data);
							console.log("else");
						}
						removeModal();
						...
						if (options.complete)
							options.complete(obj.data);
                    } else
                    	alertMessage(obj.data, 2);
                    	console.log("else2");
				},
				error: function(xhr, status, error) {
					alertMessage('por favor volver a intentar.', 2);
